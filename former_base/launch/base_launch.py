...
def generate_launch_description():
    ...
    robot_config_dir = LaunchConfiguration(
        ...
        default=os.path.join(
            get_package_share_directory("former_base"), "config", "former.yaml"
        ),
    )
    ...
    teleop_launch_file_dir = os.path.join(
        get_package_share_directory("former_teleop"), "launch"
    )
    sick_tim_launch_file_dir = os.path.join(
        get_package_share_directory("former_base"), "launch/include"
    )
    ...
    return LaunchDescription(
        [
            # Node(
            #     package="former_base",
            #     executable="former_gpio_node",
            #     name="former_gpio_node",
            #     output="screen",
            ...
            Node(
                package="controller_manager",
                executable="spawner.py",
                arguments=["former_base_controller"],
                output="screen",
            ),
            # Node(
            ...
            Node(
                ...
                parameters=[robot_config_dir],
                remappings={
                    ("/cmd_vel_out", "/former_base_controller/cmd_vel_unstamped")
                },
            ),
            Node(
                package="twist_mux",
                ...
                remappings={("/twist", "/former_base_controller/cmd_vel_unstamped")},
            ),
