...
#include "clober_hardware/clober_hardware.hpp"

namespace clober_hardware
{
hardware_interface::return_type CloberHardware::configure(const hardware_interface::HardwareInfo& info)
{
  ...
  for (const hardware_interface::ComponentInfo& joint : info_.joints)
  {
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("CloberHardware"), "Joint '%s' has %d command interfaces found. 1 expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("CloberHardware"), "Joint '%s' have %s command interfaces found. '%s' expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("CloberHardware"), "Joint '%s' has %d state interface. 2 expected.",
    ...
    {
      RCLCPP_FATAL(rclcpp::get_logger("CloberHardware"),
...
std::vector<hardware_interface::StateInterface> CloberHardware::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> CloberHardware::export_command_interfaces()
{
...
hardware_interface::return_type CloberHardware::start()
{
  ...
  // Initialize node
  node_ = std::make_shared<rclcpp::Node>("clober_hardawre_node");
  ...
  // Initialize client
  srv_led_ = node_->create_client<clober_msgs::srv::SetLed>("/led/control");

  // Initialize publisher
  pub_feedback_ = node_->create_publisher<clober_msgs::msg::Feedback>(robot_ + "/feedback", 1);
  rp_feedback_ = std::make_shared<realtime_tools::RealtimePublisher<clober_msgs::msg::Feedback>>(pub_feedback_);
  rp_feedback_->msg_.robot = robot_;

  for (size_t i = 0; i < ceil(info_.joints.size() / 2.0); i++)
  {
    rp_feedback_->msg_.motor_state.push_back(clober_msgs::msg::MotorState());
  ...
  auto req = std::make_shared<clober_msgs::srv::SetLed::Request>();
  req->color = clober_msgs::srv::SetLed::Request::MINT;
  req->mode = clober_msgs::srv::SetLed::Request::NORMAL;
  srv_led_->async_send_request(req);

  status_ = hardware_interface::status::STARTED;
  RCLCPP_INFO(rclcpp::get_logger("CloberHardware"), "System Successfully started!");
  return hardware_interface::return_type::OK;
}

hardware_interface::return_type CloberHardware::stop()
{
  status_ = hardware_interface::status::STOPPED;
  controller_->serial_->close();
  RCLCPP_INFO(rclcpp::get_logger("CloberHardware"), "System successfully stopped!");
  return hardware_interface::return_type::OK;
}

hardware_interface::return_type CloberHardware::read()
{
  controller_->read();
...
hardware_interface::return_type CloberHardware::write()
{
  for (size_t i = 0; i < info_.joints.size(); i++)
...
void CloberHardware::publishFeedback()
{
...
void CloberHardware::publishEstopState()
{
  if (estop_state_.data != previous_state_)
  {
    if (rp_estop_->trylock())
    {
      ...
      {
        auto req = std::make_shared<clober_msgs::srv::SetLed::Request>();
        req->color = clober_msgs::srv::SetLed::Request::RED;
        req->mode = clober_msgs::srv::SetLed::Request::BLINK;
        srv_led_->async_send_request(req);
      }
      else
      {
...
PLUGINLIB_EXPORT_CLASS(clober_hardware::CloberHardware, hardware_interface::SystemInterface)